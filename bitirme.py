...
FACE_CASCADE_PATH = Path(r"C:\Users\pv\prediction_image\haarcascade_frontalface_default.xml")

# =========================
# Yardımcı fonksiyonlar
# =========================
def load_face_cascade(cascade_path: Path) -> cv2.CascadeClassifier:
    cascade = cv2.CascadeClassifier(str(cascade_path))
    if cascade.empty():
        raise FileNotFoundError(f"Cascade dosyası yüklenemedi: {cascade_path}")
    return cascade
...
def detect_and_crop_first_face(
    image_path: Path,
...
def load_yawn_data(
    ...
    for class_name in ["yawn", "no_yawn"]:
        ...
        for image_name in os.listdir(class_dir):
            ...
            face_img = detect_and_crop_first_face(image_path, face_cascade)

            if face_img is not None:
                data.append((face_img, class_index))
    return data
...
def build_dataset(
    ...
    yawn_data = load_yawn_data(dataset_dir, face_cascade)
...
# =========================
def main():
    X, y = build_dataset(DATASET_DIR, FACE_CASCADE_PATH)
    ...
    print("\nModel kaydedildi: drowsiness_model.keras")

    # Örnek tahmin
    sample_image = DATASET_DIR / "no_yawn" / "3.jpg"
